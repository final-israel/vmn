...
class VersionControlStamper(IVersionsStamper):
    ...
    def stamp_app_version(self, current_changesets):
        ...
        with open(self._app_version_history_path, 'r+') as f:
            data = yaml.safe_load(f)
            f.seek(0)
    ...
    def stamp_main_system_version(self, service_version):
        ...
        if not os.path.isfile(self._main_version_history_path, 'w+'):
        ...
        with open(self._main_version_history_path, 'r+') as f:
            data = yaml.safe_load(f)
            f.seek(0)
            f.write('# Autogenerated by vmn. Do not edit\n')
            data[old_version] = {
    ...
    def _get_app_changesets(self):
        ...
        with open(self._app_version_file, 'r') as f:
            data = yaml.safe_load(f)
            current_changesets = data['changesets']
            version = data['_version']

        if os.path.isfile(self._app_version_history_path):
            with open(self._app_version_history_path, 'r') as f:

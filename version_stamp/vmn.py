...
class IVersionsStamper(object):
    ...
    def stamp_app_version(self, user_repo_details, starting_version, release_mode=None):
        raise NotImplementedError('Please implement this method')
...
def get_version(versions_be_ifc, params):
    ...
    retries = 3
    release_mode = None
    starting_version = params['starting_version']
    ...
    while retries:
        ...
        current_version = versions_be_ifc.stamp_app_version(
            user_repo_details, starting_version, release_mode
        )
        formatted_main_ver = versions_be_ifc.stamp_main_system_version(
            current_version
        )

        err = versions_be_ifc.publish(current_version, formatted_main_ver)
        if not err:
            stamped = True
            break

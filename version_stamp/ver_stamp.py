...
class VersionControlStamper(IVersionsStamper):
    def __init__(self, conf):
        ...
        self._versions_repo_path = conf['versions_repo_path']
        ...
        if 'main_version_file' in conf:
            ...
            self._main_path = os.path.dirname(self._main_version_file)

            self._main_version_history_path = \
                os.path.join(self._main_path, '_main_version_history.py')

    def allocate_backend(self):
        self._backend, _ = stamp_utils.get_client(self._versions_repo_path)
    ...
    def stamp_app_version(self, current_changesets, extra_info=False):
        ...
        if not os.path.isfile(self._app_version_history_path):
            ...
            self._backend.commit(
                ...
                user='version_manager',
                include=[self._app_version_history_path],
            )

        custom_major = None
        custom_minor = None
        custom_repos = None
        version_info_file = os.path.join(self._app_path, 'version_info.py')
    ...
    def stamp_main_system_version(self, service_version):
        ...
        if not os.path.isfile(self._main_version_file):
            pathlib.Path(self._main_path).mkdir(parents=True, exist_ok=True)
        ...
        if not os.path.isfile(self._main_version_history_path):
            ...
            self._backend.commit(
                message='{0}: add first main history version file'.format(
                    self._main_system_name),
                user='version_manager',
                include=[self._main_version_history_path],
            )
...
def _enhance_params(versions_path, params):
    ...
    if not params['app_version_file'].startswith(versions_path):
        raise RuntimeError(
    ...
    params['app_path'] = params['app_version_file'].split('version.py')[0]

    if params['main_version_file'] is None:
        params.pop('main_version_file')
        params.pop('main_system_name')
    ...
    if not main_version_file.startswith(versions_path):
        raise RuntimeError(
    ...
    params['main_system_path'] = \
        params['main_version_file'].split('main_version.py')[0]


def run_stamper(**params):
    repos_path = params['repos_path']
    versions_repo_path = stamp_utils.get_versions_repo_path(repos_path)
    _enhance_params(versions_repo_path, params)
    ...
    with lock:
        ...
        params['versions_repo_path'] = versions_repo_path
        be = VersionControlStamper(params)

        be.allocate_backend()
        changesets = HostState.get_current_changeset(repos_path)
